      ...
      <div class="f_da wow bounceIn" style="max-height:300px;height: 280px; overflow: hidden">
        <div><img src="${pro.img}"/>
          <ul style="font-size: 17px">
            ...
            <li style="white-space: nowrap;overflow: hidden"><strong class="text_shadow">[图片简介]</strong><span class="text_shadow">${pro.introduction}</span></li>
            <li><strong class="text_shadow">[主题]</strong><span class="text_shadow">${pro.theme}</span></li>
            ...
            <c:if test="${sessionScope.user != null}">
              <c:if test="${requestScope.ct == 0}">
                <span class="icon-collection" aria-hidden="true" style="color: red">
                  <button class="Collection" onclick="location.href = '/collect?id=${pro.id}&type=1'">立即收藏</button>
                </span>
              </c:if>
              <c:if test="${requestScope.ct == 1}">
              <span class="icon-collection" aria-hidden="true" style="color: red">
                <button class="Collection" onclick="location.href = '/collect?id=${pro.id}&type=2'">取消收藏</button>
              </span>
              </c:if>
            </c:if>
          </ul>
